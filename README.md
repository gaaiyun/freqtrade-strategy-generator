...
2. **校验**：生成完不能直接信，用 AST 静态检查常见 LLM 翻车（语法错、缺方法、缺属性、未来函数嫌疑、把聚宽 API 抄过来、stoploss 写成正数、漏 freqtrade import、`minimal_roi` 缺 `"0"` 键会让 freqtrade 崩、用了已废弃的 v2 接口等），有 `FT000`–`FT010` 错误码。
...
模型和 endpoint 可用环境变量覆盖，不用改代码：`DEEPSEEK_MODEL` / `DEEPSEEK_BASE_URL`（openai / anthropic 同理 `*_MODEL` / `*_BASE_URL`）。比如 deepseek 换模型只要 `export DEEPSEEK_MODEL=deepseek-v4-flash`。`list-backends` 显示的是这样解析后的真实模型名。
...
| FT006 | error | `stoploss` 是正数（Freqtrade 约定必须负数）；`stoploss: float = 0.05` 这种带注解的也查 |
| FT007 | warning | 看不到 `from freqtrade.* import ...` |
| FT008 | error | `minimal_roi` 缺 `"0"` 键 — 没它 freqtrade 在 ROI 检查时 `max()` 空列表直接崩（freqtrade#1633） |
| FT009 | warning | `minimal_roi` 键写成整数 `{0: ...}` — freqtrade 约定用字符串分钟数 `"0"` |
| FT010 | error | 用了已废弃的 v2 接口 `populate_buy_trend` / `populate_sell_trend`，v3 应改名 `populate_entry_trend` / `populate_exit_trend` |
...
│   ├── strategy_validator.py   # v2：AST 静态校验（FT000-FT010）
│   └── strategy_generator.py   # v1：硬编码模板生成器
├── tests/
│   ├── test_llm_generator.py       # mock-LLM 测试，不发真请求
│   └── test_strategy_validator.py  # 用刻意 bad code 触发各错误码的测试
├── references/
│   └── .env.example            # API key / 模型 / endpoint 环境变量样例
├── SKILL.md
├── README.md
└── requirements.txt
```
...
测试全过（53 个），全部 mock，不发真 LLM 请求；CI 在 GitHub Actions 上跑同一套。
...
- 默认 `temperature=0.2` 让生成结果偏稳定；想拿同一描述生成几个变体，加 `--temperature 0.8` 调高即可。
